...
# 转化规则（隐式转化规则）
    ...
    2.字符串与数字比较，字符串与布尔值比较，数字与布尔值比较，两侧转化成数字再比较
    3.undefined 与 null 比较特殊,要比较相等性之前,不能将 null 和 undefined 转换成其他任何值
        undefiend == undefined　　 //　true
        undefined == null　　      //　true
        null == null　　           //　true
        undefined == 其他值　　    //　false
        null == 其他值　　         //　false  
    4.NaN（非数值）也很特殊，NaN 和任何值（包括自己）比较都是返回 false
    5.等号两侧为相同的引用数据类型时，由于引用类型保存的是对象（包括数组，函数）的地址值，所以地址值不同的，返回的都是 false
    6.只有一侧为引用数据类型时，都转化成数字
        [1，2] == 1　　   ==>　　NaN == 1　　      //　false
